...
export default function Scroll({
  ...
  function move(e: MouseEvent<HTMLDivElement>) {
    ...
    if (onMove) {
      onMove(
        scrollBarActive.current?.offsetWidth / scrollArea.current.offsetWidth,
      );
    }
  }
    ...
    >
      <div className="h-1 bg-text-dim w-full rounded-full">
        <div
          ref={scrollBarActive}
          className={cn(
            "bg-white h-full flex items-center",
            "relative rounded-full group-hover:bg-accent",
          )}
        >
          <div
            className={cn(
              "size-3 rounded-full bg-white right-0",
              "translate-x-1/2 absolute hidden group-hover:block",
            )}
          ></div>
        </div>
      </div>
